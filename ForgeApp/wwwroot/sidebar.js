async function getJSON(url) {
    ...
    if (!resp.ok) {
        alert('Could not loa d tree data. See console for more details.')
...
async function getHubs() {
    const projects = await getJson(`/api/hubs`);
    return hubs.map(hub => createTreeNode(`hub|${hubId}`, hub.attributes.name, 'icon-hub', true));
}
